...
class TestAdminPage:
    ...
    def test_admin_page(self):
        self.client = Client()
        response = self.client.post("/admin/login/", {"username": "john", "password": "smith"})
        ...
        assert b'<option selected value="?is_staff__exact=1">Yes</option>' in response.content
    ...
    def test_simple_list_filter(self):
        ...
        custom_list_filter = CustomListFilter(request, {"is_staff": '1'}, model=User, model_admin=model_admin)
        ...
        assert custom_list_filter.lookups(request, model_admin) == (('1', 'Yes'), ('0', 'No'))
        assert custom_list_filter.queryset(request, User.objects.all()).count() == 1
        assert custom_list_filter.queryset(request, User.objects.all())[0].username == 'john'

        custom_list_filter = CustomListFilter(request, {"is_staff": '0'}, model=User, model_admin=model_admin)
